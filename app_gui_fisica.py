...
class AppFisica(ctk.CTk):
    ...
    def _render_respuesta(self, texto):
        parts = FORMULA_BLOCK.split(texto)
        for i, part in enumerate(parts):
            if i % 2 == 0:
                self.texto_salida.insert("end", part)
            else:
                latex = part.strip()
                try:
                    photo = self.formula_renderer.render(latex)
                    self.texto_salida.insert("end", "\n  ")
                    self.texto_salida.image_create("end", image=photo)
                    self.texto_salida.insert("end", "\n")
                except Exception:
                    self.texto_salida.insert("end", f"\n  [ {latex} ]\n")
        self.texto_salida.see("end")
